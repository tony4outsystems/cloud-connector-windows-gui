...
internal sealed class SelfUpdateManager
{
    ...
    private const string ReleaseAssetName = "cloud-connector-windows-gui-win-x64.zip";

    private readonly GitHubReleaseClient releaseClient;

    public SelfUpdateManager()
        : this(new GitHubReleaseClient(RepositoryOwner, RepositoryName, "No stable cloud-connector-windows-gui release was found."))
    {
    }
    ...
    public async Task ApplyUpdateAndRestartAsync(SelfUpdateStatus status, IProgress<string>? progress = null, CancellationToken cancellationToken = default)
    {
        ...
        progress?.Report($"Downloading cloud-connector-windows-gui {status.LatestVersion}...");
        var archivePath = Path.Combine(Path.GetTempPath(), $"{Guid.NewGuid():N}-{status.Asset.Name}");
        var stagingDirectory = Path.Combine(Path.GetTempPath(), $"cloud-connector-windows-gui-{Guid.NewGuid():N}");
